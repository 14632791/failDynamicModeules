...
namespace Metro.DynamicModeules.BLL
{
    ...
    public class AttachTool
    {
        ...
        {
            string fileType = sourceRow[TblAttachFile.FileType].ToString();
            ...
            if (writeRow)
            {
                ...
                sourceRow[TblAttachFile.IconLarge] = ms1.ToArray();
                ms1.Close();

                MemoryStream ms2 = new MemoryStream();
                small.ToBitmap().Save(ms2, ImageFormat.Bmp);
                sourceRow[TblAttachFile.IconSmall] = ms2.ToArray();
                ms2.Close();
    ...
    public class AttachmentStorage_SQL : IAttachmentStorage
    {
        ...
        public void DoPrepareDataSource()
        {
            ...
            //临时显示字段
            if (_attachmentStorage.Columns.IndexOf(TblAttachFile.IconLarge) < 0)
                _attachmentStorage.Columns.Add(TblAttachFile.IconLarge, typeof(byte[]));

            if (_attachmentStorage.Columns.IndexOf(TblAttachFile.IconSmall) < 0)
                _attachmentStorage.Columns.Add(TblAttachFile.IconSmall, typeof(byte[]));
            ...
            foreach (DataRow R in _attachmentStorage.Rows)
            {
                ...
                if (R[TblAttachFile.IconSmall] != DBNull.Value) continue;
        ...
        private byte[] GetFile(string fileName)
        {
            ...
            if (rows.Length > 0)
                return (byte[])rows[0][TblAttachFile.FileBody];
            else
                return null;
        }
        /// <summary>
        ...
        public void AddFile(object file)
        {
            ...
            row[TblAttachFile.DocID] = "";
            row[TblAttachFile.FileTitle] = f.FileTitle;
            row[TblAttachFile.FileName] = f.FileName;
            row[TblAttachFile.FileType] = f.FileType;
            row[TblAttachFile.FileSize] = f.FileSize;
            row[TblAttachFile.FileBody] = f.FileBody;
            row[TblAttachFile.IsDroped] = f.IsDroped;
            row[TblAttachFile.IconLarge] = f.IconLarge;
            row[TblAttachFile.IconSmall] = f.IconSmall;
        ...
        private void DoOpenFile(object attachmentDataRow)
        {
            try
            {
                ...
                byte[] file = (byte[])R[TblAttachFile.FileBody]; //取文件内容
                string tempFile = CreateTempFile(file as byte[], R[TblAttachFile.FileName].ToString());
