...
namespace Metro.DynamicModeules.BLL.Business
{
    ...
    public class BllIN : BllBaseBusiness, IFuzzySearchSupportable
    {
        ...
        public BllIN()
        {
            _KeyFieldName = TblIN.__KeyName;
            _SummaryTableName = TblIN.__TableName;
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[TblIN.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[TblIN.INNO] = "*自动生成*";
            row[TblIN.DocDate] = DateTime.Today;
            row[TblIN.DocUser] = Loginer.CurrentUser.Account;
            row[TblIN.CreatedBy] = Loginer.CurrentUser.Account;
            row[TblIN.CreationDate] = DateTime.Now;
            row[TblIN.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[TblIN.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override void ApprovalBusiness(DataRow summaryRow)
        {
            ...
            string key = ConvertEx.ToString(summaryRow[TblIN.INNO]);
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, TblIN.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[TblINs.__TableName].Copy();
        ...
        public DataTable GetSummaryByParam(string docNoFrom, string docNoTo, DateTime docDateFrom, DateTime docDateTo,OrderStatus status=0)
        {
            return _Bridge.GetSummaryByParam(
                ...
                Globals.RemoveInjection(docNoTo, 20), docDateFrom, docDateTo, (int)status);
        }

        //public override void WriteLog()
        //{
        //    string key = this.DataBinder.Rows[0][TblIN.INNO].ToString();//取单据号码
        ...
        //        logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], TblIN.__TableName, TblIN.__KeyName, true); //主表
        //        logBridge.WriteLog(logID, original.Tables[1], changes.Tables[1], TblINs.__TableName, TblINs.__KeyName, false);//明细
        //    }
        //    catch
        //    {
