...
using Metro.DynamicModeules.Bridge.PurchaseModule;
...
namespace Metro.DynamicModeules.BLL.Business
{
    ...
    {
        private IBridgePO _Bridge = null;
        ...
        public BllPO()
        {
            _KeyFieldName = TblPO.__KeyName; //主键字段
            _SummaryTableName = TblPO.__TableName;//表名         
            _Bridge = this.CreateBridge();//实例化桥接功能
        }

        private IBridgePO CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                return new ADODirectPO().GetInstance();

            if (BridgeFactory.BridgeType == BridgeType.WebService)
                return new WebServicePO();
        ...
        public override void ApprovalBusiness(DataRow summaryRow)
        {
            ...
            string key = ConvertEx.ToString(summaryRow[TblPO.__KeyName]);
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[TblPO.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[TblPO.PONO] = "*自动生成*";
            row[TblPO.PODate] = DateTime.Now;
            row[TblPO.POUser] = Loginer.CurrentUser.Account;
            row[TblPO.FlagApp] = "N";
            row[TblPO.CreatedBy] = Loginer.CurrentUser.Account;
            row[TblPO.CreationDate] = DateTime.Now;
            row[TblPO.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[TblPO.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, TblPO.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[TblPOs.__TableName].Copy();
            this.UpdateDetailCommonValue(detail); //更新明细表的公共字段数据
            save.Tables.Add(detail); //加入明细数据 

            //附件独立保存
            //if (currentBusiness.Tables[TblAttachFile.TableName] != null)
            //{
            //    DataTable attach = currentBusiness.Tables[TblAttachFile.TableName].Copy();
        ...
        //{
        //    string key = this.DataBinder.Rows[0][TblPO.__KeyName].ToString();//取单据号码
        ...
        //        logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], TblPO.__TableName, TblPO.__KeyName, true); //主表
        //        //明细表的修改日志,系统不支持自动生成,请手工调整代码
        //        //SystemLog.WriteLog(logID, original.Tables[1], changes.Tables[1], TblPO.__TableName, TblPO.__KeyName, false);
        //    }
        //    catch
        //    {
