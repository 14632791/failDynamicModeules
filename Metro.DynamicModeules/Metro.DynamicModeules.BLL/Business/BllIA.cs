...
namespace Metro.DynamicModeules.BLL.Business
{
    ...
    {
        private IBridgeIA _Bridge = null;

        public BllIA()
        {
            _KeyFieldName = TblIA.__KeyName;
            _SummaryTableName = TblIA.__TableName;
            _Bridge = this.CreateBridge();//实例化桥接功能
        }

        private IBridgeIA CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                return new Metro.DynamicModeules.Server.DataAccess.DalBusiness.DalIA(Loginer.CurrentUser);//new ADODirect_IA().GetInstance();
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[TblIA.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[TblIA.IANO] = "*自动生成*";
            row[TblIA.DocDate] = DateTime.Today;
            row[TblIA.DocUser] = Loginer.CurrentUser.Account;
            row[TblIA.CreatedBy] = Loginer.CurrentUser.Account;
            row[TblIA.CreationDate] = DateTime.Now;
            row[TblIA.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[TblIA.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override void ApprovalBusiness(DataRow summaryRow)
        {
            ...
            string key = ConvertEx.ToString(summaryRow[TblIA.IANO]);
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, TblIA.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[TblIAs.__TableName].Copy();
        ...
        //{
        //    string key = this.DataBinder.Rows[0][TblIA.IANO].ToString();//取单据号码
        ...
        //        logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], TblIA.__TableName, TblIA.__KeyName, true); //主表
        //        logBridge.WriteLog(logID, original.Tables[1], changes.Tables[1], TblIAs.__TableName, TblIAs.__KeyName, false);//明细
        //    }
        //    catch
        //    {
