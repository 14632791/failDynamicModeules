...
namespace Metro.DynamicModeules.BLL.Business
{
    ...
    {
        private IBridgeAR _Bridge = null;

        public BllAR()
        {
            _KeyFieldName = TblAR.__KeyName;
            _SummaryTableName = TblAR.__TableName;
            _Bridge = this.CreateBridge();//实例化桥接功能
        }

        private IBridgeAR CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                return new ADODirectAR().GetInstance();

            if (BridgeFactory.BridgeType == BridgeType.WebService)
                return new WebServiceAR();
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[TblAR.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[TblAR.__KeyName] = Globals.DEF_NO_TEXT;
            row[TblAR.ReceivedDate] = DateTime.Today;
            row[TblAR.Currency] = Globals.DEF_CURRENCY;
            row[TblAR.CreatedBy] = Loginer.CurrentUser.Account;
            row[TblAR.CreationDate] = DateTime.Now;
            row[TblAR.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[TblAR.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override void ApprovalBusiness(DataRow summaryRow)
        {
            ...
            string key = ConvertEx.ToString(summaryRow[TblAR.__KeyName]);
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, TblAR.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[TblARs.__TableName].Copy();
        ...
        //{
        //    string key = this.DataBinder.Rows[0][TblAR.ARNO].ToString();//取单据号码
        ...
        //        logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], TblAR.__TableName, TblAR.__KeyName, true); //主表
        //        logBridge.WriteLog(logID, original.Tables[1], changes.Tables[1], TblARs.__TableName, TblARs.__KeyName, false);//明细
        //    }
        //    catch
        //    {
